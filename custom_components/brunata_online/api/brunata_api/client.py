...
class BrunataClient:
    ...
    def __init__(
        self, username: str, password: str, session: ClientSession, locale: str
    ) -> None:
    ...
    async def _update_metadata(self, locale: str = "en") -> None:
        ...
        if mapping_config.is_error():
            raise RuntimeError(
                f"Failed to retrieve mapping configuration: {mapping_config.value}"
            )

        allocation_units = await self.api.get_allocation_units()
        if allocation_units.is_error():
            raise RuntimeError(
                f"Failed to retrieve allocation units: {allocation_units.value}"
            )
    ...
    async def get_consumption(
        ...
        consumption = await self.api.get_consumption(
            start_date, end_date, _type, unit, interval
        )
